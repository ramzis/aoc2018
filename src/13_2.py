    ...
    def solve(self):
        ...
        while len(carts) > 1:
            ...
            for c in range(len(carts)):
                ...
                if (next_x, next_y) in [cart[0] for cart in carts if cart != ()]:
                    carts = list(map(
                        lambda x: () if (x == carts[c] or x[0] == (next_x, next_y)) else x, carts))
                    continue
            ...
            carts = sorted([c for c in carts if c != ()],
                           key=lambda x: (x[0][1], x[0][0]))

        self.solution = "{},{}".format(carts[0][0][0], carts[0][0][1])
